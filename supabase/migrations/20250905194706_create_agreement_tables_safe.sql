-- Arkansas Ankle Monitor Agreement Database Schema (Safe Version)
...
-- Drop trigger if it exists, then create it
DROP TRIGGER IF EXISTS audit_agreement_submissions ON agreement_submissions;
...
DROP POLICY IF EXISTS "Allow all operations on agreement_submissions" ON agreement_submissions;
CREATE POLICY "Allow all operations on agreement_submissions" ON agreement_submissions
    FOR ALL USING (true);

DROP POLICY IF EXISTS "Allow all operations on file_uploads" ON file_uploads;
